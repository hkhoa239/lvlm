...
def get_data(args, row):
    if args.dataset == 'MMVP':
        ...
        if isinstance(img_field, dict):
            # HF datasets / parquet format: {'bytes': ..., 'path': ...}
            raw = img_field.get('bytes')
            if raw is None and img_field.get('path'):
                image = Image.open(img_field['path']).convert('RGB')
            else:
                image = Image.open(BytesIO(raw)).convert('RGB')
        elif isinstance(img_field, (bytes, bytearray)):
            image = Image.open(BytesIO(img_field)).convert('RGB')
...
def save_heatmaps(masks, images, size, index, index_o, outdir, model_name, box, classes, labels, out=224):
    ...
    up = torch.nn.UpsamplingBilinear2d(size=images.shape[-2:]).to(DEVICE)
